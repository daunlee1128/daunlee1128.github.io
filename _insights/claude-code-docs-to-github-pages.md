...
3. 이 repo를 clone했다면 커밋된 유출 검사 훅을 켠다. 끄려면 아래 마지막 줄을 쓴다.
...
이 글은 2026-09-13 수정(커밋 `287039b`)에서 이 09-07 기록을 보탰다.
...
버전은 2026-09-15 `Gemfile.lock`으로 확인했다.
로컬 `Gemfile.lock`은 미추적 파일이고, 그 버전이 GitHub Pages 기본 빌드 환경을 결정하지 않는다.
...
[GitHub Pages 플랜 안내](https://docs.github.com/en/pages/getting-started-with-github-pages/what-is-github-pages)를 확인했다(2026-09-15).
...
[GitHub Pages 접근 제어 문서](https://docs.github.com/en/enterprise-cloud@latest/pages/getting-started-with-github-pages/changing-the-visibility-of-your-github-pages-site)에 따른 범위다(2026-09-15).
...
이 재검증에 쓴 Git은 2.50.1이다.
처음에는 최신 트리만 검사했다(커밋 `9e1b35b`).
그러면 누출 파일을 다음 커밋에서 지운 뒤 push할 때 놓친다.
원격 이력에는 이전 blob이 남아서 검사 범위를 커밋 전체로 바꿨다(커밋 `5299518`).
회귀 테스트 `test_range_mode_catches_blob_deleted_in_later_commit`으로 재현했다.
누출 파일을 커밋한 뒤 지우면, 팁 트리 검사(`--tree`)는 통과하지만 범위 검사(`--range`)는 그 파일을 여전히 잡는다.

<details markdown="1">
<summary>훅을 검토하려면 펼치기: 첫 push와 브랜치 삭제를 포함한 전체 코드</summary>

아래는 이 repo에 커밋된 `.githooks/pre-push`다.
2026-09-15에 HEAD와 로컬 사본이 같은지 확인했다.
...
설정법을 순서대로 설명하는 글과 달리, 이 글은 파일을 골라 옮긴 범위만 다룬다.
볼트 전체를 사이트로 바꾸는 자동 미러링은 하지 않았다.
