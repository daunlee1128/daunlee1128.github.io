...
답은 [“호출이 proxy를 지나도록 호출 구조를 나눴다” 절](#fix)에 있다.
...
## 같은 bean 안의 호출이 proxy를 거치지 않았다

원인은 같은 bean 내부 호출이었다. 그 호출이 Spring AOP proxy를 거치지 않아 `@Transactional`이 적용되지 않았다.

Spring Framework 7.0.9 reference는 proxy 모드(기본값)에서 proxy로 들어온 외부 호출만 가로챈다고 적는다.
self-invocation으로 부른 메서드에는 `@Transactional`이 붙어 있어도 트랜잭션 advice가 적용되지 않는다.
출처는 [Using @Transactional][tx] 문서다(2026-09-14 확인).
...
`@Async`도 기본 advice mode가 `proxy`다. 이 모드는 같은 클래스 안의 local call을 가로채지 못한다.
출처는 7.0.9 reference의 [Task Execution and Scheduling][async] 문서다(2026-09-14 확인).

그래서 규칙을 세웠다. Spring의 proxy 모드에서는 `this`로 부른 self-invocation에 그 메서드의 advice가 적용되지 않는다.
근거는 7.0.9 reference의 [Proxying Mechanisms][proxying] 문서다(2026-09-14 확인).
